...
<template>
	<div class="lottery_body">
		<div class="body_bg">
			<image mode="widthFix" src="https://image.etcchebao.com/etc-min/lottery-body-bg-2.png" />
		</div>
		<div class="lottery_turntable">
			<image class="main_bg" src="https://image.etcchebao.com/etc-min/lottery-mian.png"
				:style="{ transform: rotate_deg, transition: rotate_transition }" />
			<div class="btn_box">
				<image class="pointer" src="https://image.etcchebao.com/etc-min/lottery-btn-1.png" v-if="lotteryNum > 0"
					@click="start" />
				<image class="pointer" src="https://image.etcchebao.com/etc-min/lottery-btn-2.png" v-else />
				<image class="turntable" src="https://image.etcchebao.com/etc-min/lottery-btn-2.png" />
			</div>
		</div>
	...
	export default {
		...
		async mounted() {
			...
			if (this.token) {
				...
				uni.navigateTo({
					url: `/pages/winning/main?orderSource=${encodeURIComponent(orderSource)}`,
				});
			}
		},
		methods: {
