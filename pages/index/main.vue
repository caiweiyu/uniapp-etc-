...
export default {
  ...
  async mounted() {
    let {
      ...
      inviterUserId = "",
      orderSource = "",
      backgroundImage = "https%3A%2F%2Fimage.etcchebao.com%2F20201105151649sdQssA.png",
    } = this.$root.$mp.query;
  ...
  methods: {
    ...
    toVip() {
      let src = encodeURIComponent(
        `${chewu}/vip/dist/#/index?token=${this.token}&city=&lng=${this.longitude}&lat=${this.latitude}&activityId=${this.activityId}&inviterUserId=${this.inviterUserId}`
      );
      if (this.orderSource) {
        src = encodeURIComponent(
          `${chewu}/vip/dist/#/index?token=${this.token}&city=&lng=${this.longitude}&lat=${this.latitude}&activityId=${this.activityId}&inviterUserId=${this.inviterUserId}&from=${this.orderSource}`
        );
      }
      uni.navigateTo({
